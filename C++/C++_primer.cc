...
class Sales_data {
  ...
  // 类的构造函数，不定义的话有一个默认构造函数，用类内初始值，没有则默认初始化
  Sales_data() = default;    // 需要一个默认构造函数
  ...
  Sales_data(istream &); // 以istream为参数的构造函数需要在{}内执行一些是自己操作，可以在内声明，在外定义
};
...
//linline,函数前加上inline即成为内联函数，在编译时展开，开销小，把内联和constexpr函数定义在头文件
 ...
 */
  //成员函数的声明必须在类的内部，定义可以在内部也可以在外部，接口组成部分的非成员函数的定义和声明都在类的外部
  struct Sales_data{
    //数据成员（放在成员函数后也可以，编译器先编译成员，再编译函数）
    string bookNo;
    unsigned units_sold = 0;
    double revenue = 0.0;
    Sales_data avg_price() const;
    //成员函数，p231，额外的this隐式参数（调用函数的对象的地址）帮助函数得到调用对象的地址，从何访问其数据成员
    string isbn() const {return bookNo;}  //const把this指针改为指向常量（原来不是指向常量）的常量指针
    //如果不加const导致无法访问const的对象（不能用一个常量对象的地址（const的对象）初始化一个指向非常量的常量指针（this））
    //这样使用const的成员函数称作常量成员函数
  };
