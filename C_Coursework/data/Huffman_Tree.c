...
//构建哈夫曼树
HuffmanTree CreateHuff(DataType *w, int n)
{
    int m = 2 * n - 1;                               //哈夫曼树总结点数
    HuffmanTree HT = (HuffmanTree)calloc(m + 1, sizeof(HTNode)); //开m+1个HTNode，因为下标为0的HTNode不存储数据
    for (int i = 1; i <= n; i++){
...
//生成哈夫曼编码
HuffmanCode HuffCoding(HuffmanTree HT, int n){
    HuffmanCode HC = (HuffmanCode)malloc(sizeof(char *) * (n + 1)); //开n+1个空间，因为下标为0的空间不用
...
int main(){
    ...
    HuffmanTree HT = CreateHuff(w, n); //构建哈夫曼树

    HuffmanCode HC = HuffCoding(HT, n); //构建哈夫曼编码
    // 打印哈夫曼编码
    for (int i = 1; i <= n; i++) {
        printf("数据%.2lf的编码为:%s\n", HT[i].weight, HC[i]);
    }
